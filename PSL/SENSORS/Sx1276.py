...
class SX1276:
    ...
    def __init__(self, SPI, frq, **kwargs):
        ...
        self.setTxPower(
            kwargs.get("power", 17),
            self.PA_OUTPUT_PA_BOOST_PIN
            if kwargs.get("boost", True)
            else self.PA_OUTPUT_RFO_PIN,
        )
        self.idle()
    ...
    def parsePacket(self, size=0):
        ...
        if (irqFlags & self.IRQ_RX_DONE_MASK) and (
            irqFlags & self.IRQ_PAYLOAD_CRC_ERROR_MASK
        ) == 0:
            self._packetIndex = 0
            ...
            self.SPIWrite(
                self.REG_FIFO_ADDR_PTR, self.SPIRead(self.REG_FIFO_RX_CURRENT_ADDR, 1)
            )
            self.idle()
        elif self.SPIRead(self.REG_OP_MODE)[0] != (
            self.MODE_LONG_RANGE_MODE | self.MODE_RX_SINGLE
        ):
            self.SPIWrite(self.REG_FIFO_ADDR_PTR, [0])
            self.SPIWrite(
                self.REG_OP_MODE, [self.MODE_LONG_RANGE_MODE | self.MODE_RX_SINGLE]
            )
        return self.packetLength

    def packetRssi(self):
        return self.SPIRead(self.REG_PKT_RSSI_VALUE)[0] - (
    ...
    def setSpreadingFactor(self, sf):
        ...
        self.SPIWrite(
            ...
            [(self.SPIRead(self.REG_MODEM_CONFIG_2)[0] & 0x0F) | ((sf << 4) & 0xF0)],
        )
    ...
    def setSignalBandwidth(self, sbw):
        ...
        self.SPIWrite(
            ...
            [(self.SPIRead(self.REG_MODEM_CONFIG_1)[0] & 0x0F) | (bw << 4)],
        )
    ...
    def setCodingRate4(self, denominator):
        ...
        self.SPIWrite(
            self.REG_MODEM_CONFIG_1,
            [
                (self.SPIRead(self.REG_MODEM_CONFIG_1)[0] & 0xF1) |
                ((denominator - 4) << 4)
            ],
        )
    ...
    def crc(self):
        self.SPIWrite(
            self.REG_MODEM_CONFIG_2, [self.SPIRead(self.REG_MODEM_CONFIG_2)[0] | 0x04]
        )

    def noCrc(self):
        self.SPIWrite(
            self.REG_MODEM_CONFIG_2, [self.SPIRead(self.REG_MODEM_CONFIG_2)[0] & 0xFB]
        )
    ...
    def explicitHeaderMode(self):
        ...
        self.SPIWrite(
            self.REG_MODEM_CONFIG_1, [self.SPIRead(self.REG_MODEM_CONFIG_1)[0] & 0xFE]
        )

    def implicitHeaderMode(self):
        self._implicitHeaderMode = 1
        self.SPIWrite(
            self.REG_MODEM_CONFIG_1, [self.SPIRead(self.REG_MODEM_CONFIG_1)[0] | 0x01]
        )
...
if __name__ == "__main__":
    ...
    lora = SX1276(
        I.SPI, 434e6, boost=True, power=17, BW=125e3, SF=12, CR=5
    )  # settings for maximum range
    lora.crc()
    cntr = 0
    while 1:
        time.sleep(0.01)
        if mode == TX:
            ...
            print(
                time.ctime(),
                [ord(a) for a in ":"] + [cntr],
                hex(lora.SPIRead(lora.REG_OP_MODE)[0]),
            )
            lora.endPacket()
            cntr += 1
            if cntr == 255:
