...
class ADS1115:
    ...
    sdr_selection = OrderedDict(
        [
            (8, REG_CONFIG_DR_8SPS),
            (16, REG_CONFIG_DR_16SPS),
            (32, REG_CONFIG_DR_32SPS),
            (64, REG_CONFIG_DR_64SPS),
            (128, REG_CONFIG_DR_128SPS),
            (250, REG_CONFIG_DR_250SPS),
            (475, REG_CONFIG_DR_475SPS),
            (860, REG_CONFIG_DR_860SPS),
        ]
    )  # sampling data rate
    ...
    def __init__(self, I2C, **args):
        ...
        self.params = {
            "setGain": self.gains.keys(),
            "setChannel": self.type_selection.keys(),
    ...
    def readADC_SingleEnded(self, chan):
        ...
        time.sleep(1.0 / self.rate + 0.002)  # convert to mS to S
        return (
            self.readRegister(self.REG_POINTER_CONVERT) * self.gain_scaling[self.gain]
        )
    ...
    def readADC_Differential(self, chan="01"):
        ...
        time.sleep(1.0 / self.rate + 0.002)  # convert to mS to S
        return (
            int16(self.readRegister(self.REG_POINTER_CONVERT)) *
            self.gain_scaling[self.gain]
        )
