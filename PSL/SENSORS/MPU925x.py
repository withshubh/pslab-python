...
class MPU925x:
    ...
    def __init__(self, I2C, **args):
        ...
        self.params = {
            ...
            "KalmanFilter": [0.01, 0.1, 1, 10, 100, 1000, 10000, "OFF"],
        }
    ...
    def KalmanFilter(self, opt):
        ...
        for a in range(500):
            ...
            for b in range(self.NUMPLOTS):
                noise[b].append(vals[b])

        self.K = [None] * 7
        for a in range(self.NUMPLOTS):
            sd = std(noise[a])
            self.K[a] = KalmanFilter(1.0 / opt, sd ** 2)
            ...
            if len(vals) == 14:
                ...
                for a in range(3):
                    raw[a] = (
                        ...
                        self.ACCEL_SCALING[self.AR]
                    )
                for a in range(4, 7):
                    raw[a] = (
                        1.0 *
                        int16(vals[a * 2] << 8 | vals[a * 2 + 1]) /
                        self.GYRO_SCALING[self.GR]
                    )
    ...
    def getMag(self):
        ...
        """
        vals = self.I2C.readBulk(
            self.AK8963_ADDRESS, 0x03, 7
        )  # 6+1 . 1(ST2) should not have bit 4 (0x8) true. It's ideally 16 . overflow bit
        ax = int16(vals[0] << 8 | vals[1])
        ay = int16(vals[2] << 8 | vals[3])
        az = int16(vals[4] << 8 | vals[5])
        if not vals[6] & 0x08:
    ...
    def WhoAmI(self):
        ...
        if v not in [0x71, 0x73]:
            return "Error %s" % hex(v)
    ...
    def initMagnetometer(self):
        ...
        self.I2C.writeBulk(
            self.AK8963_ADDRESS, [self.AK8963_CNTL, (1 << 4) | 6]
        )  # mode   (0=14bits,1=16bits) <<4 | (2=8Hz , 6=100Hz)
